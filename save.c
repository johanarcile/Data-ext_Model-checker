...
int save_to_fbs(State_space_TA *ss, const char *filename, char **locations, int nb_locations, char **actions, int nb_actions)
{
    ...
    for (int i = 0; i < nb_locations; i++)
        flatbuffers_string_vec_push(&B, flatbuffers_string_create_str(&B, locations[i]));
    flatbuffers_string_vec_ref_t locations_ref = flatbuffers_string_vec_end(&B);

    flatbuffers_string_vec_start(&B);
    for (int i = 0; i < nb_actions; i++)
        flatbuffers_string_vec_push(&B, flatbuffers_string_create_str(&B, actions[i]));
    flatbuffers_string_vec_ref_t actions_ref = flatbuffers_string_vec_end(&B);

    State_space_ta_State_space_ta_create_as_root(&B, etats_ref, succ_ref, pred_ref, nb_succ_ref, nb_pred_ref, locations_ref, actions_ref);
