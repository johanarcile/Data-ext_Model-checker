...
void init_ta() { //CAN BE OPTIMIZED BY #define NB_LOCATIONS AND NB_ACTIONS, AND HAVING ALL VARIABLES BE ARRAYS
    ...
    // Invariants
    static DBM i_0 = {{0,0,0},{16,0,infty},{16,infty,0}};
    static DBM i_1 = {{0,0,0},{16,0,infty},{16,infty,0}};
    static DBM i_2 = {{0,0,0},{16,0,infty},{16,infty,0}};
    static DBM i_3 = {{0,0,0},{16,0,infty},{16,infty,0}};
    invariants[0] = &i_0;
    invariants[1] = &i_1;
    invariants[2] = &i_2;
    invariants[3] = &i_3;
...
Variable update_a(Variable variable) {
    if (variable.v + 2 <= 30 && variable.v + 2 >= -10){ 
        variable.v += 2;
        variable.x *= 2;
...
Variable update_b(Variable variable) {
    if (variable.v *2 <= 30 && variable.v + 1 >= -10){
        variable.v *= 2;
        variable.x += 2;
        variable.active = true;
        //  variable.table[0]++;
...
Variable update_c(Variable variable) {
    if (variable.v + 1 <= 30 && variable.v * 2 >= -10) {
        variable.v += 1;
        variable.x += 1;
        variable.active = true;
        //   variable.table[0]++;
...
bool const_a(Variable variable) { return true; }
bool const_b(Variable variable) { return true; }
bool const_c(Variable variable) { return true; }
